...
/**
 * Resilient Gemini caller with automatic retry & fallback model support.
 * Gracefully handles temporary 503 (high demand) and 429 spikes.
 */
...
// Feature: Upload JD File & Intelligent Requirement Extraction
...
Extract and return ONLY a valid JSON object strictly matching this schema:
{
  ...
  "extractedText": string (clean text representation of the job description)
    ...
    // Heuristic parser fallback if Gemini key missing, offline, or rate-limited
...
// Feature: Upload Resume File & Intelligent Candidate Profile Extraction
  ...
  try {
    ...
    if (base64Data) {
      try {
        ...
        if (fastApiRes.ok) {
          const fastApiData: any = await fastApiRes.json();
        ...
        // FastApi offline or timeout; proceed to Gemini or heuristic
      }
    }
        ...
        if (parsed.name && Array.isArray(parsed.skills) && parsed.skills.length > 0) {
          const candidateData = {
            ...
            education: {
              ...
              gpa: parsed.education?.gpa || undefined,
            },
            ...
            formatCharacteristics: {
              formatType: "clean-structured" as const,
...
// Bonus Feature 1: JD Bias & Exclusionary Phrasing Analysis
...
Respond ONLY with valid JSON conforming to this schema:
{
  ...
  "improvedJD": string (an inclusive, rewritten version of the JD preserving technical rigor while removing artificial barriers)
    ...
    // Heuristic rule-based fallback if no Gemini key or temporary API spike
...
// Bonus Feature 2: Recruiter Natural-Language Query Layer
...
// Feature: Deep Candidate Resume Analysis against specific JD (Pros, Cons, Overview, Recruiter Hiring Pitch)
...
function runHeuristicCandidateAnalysis(candidate: any, jd: any) {
  const c = candidate;
  ...
  const reqSkills = (jd.requiredSkills || []) as string[];
  const prefSkills = (jd.preferredSkills || []) as string[];

  const matchedRequired = reqSkills.filter(req => {
    const r = req.toLowerCase();
    return candSkills.some((s: string) => s.includes(r) || r.includes(s)) || rawLower.includes(r);
  });
  ...
  const matchedPreferred = prefSkills.filter(pref => {
    ...
    return candSkills.some((s: string) => s.includes(p) || p.includes(s)) || rawLower.includes(p);
  });
  ...
  const keyDifferentiator = c.projects && c.projects.length >= 2
    ? `Multi-project software portfolio (${c.projects.map((p: any) => p.title).slice(0, 2).join(" & ")}) providing tangible proof-of-work.`
    : `Direct technical stack overlap with ${matchedRequired.slice(0, 3).join(", ") || "core web stack"}.`;

  const rampUpReadiness = finalScoreEst >= 75
    ? `Immediate (Days 1-5) on primary ${matchedRequired.slice(0, 2).join(" and ") || "coding"} tasks; ~1 week to acclimate to team deployment standards.`
    : `~1 to 2 weeks onboarding; rapid ramp on ${matchedRequired.slice(0, 2).join(", ") || "core stack"} with pairing on ${missingRequired[0] || "secondary tools"}.`;

  const interviewProbeQuestions = [
    c.projects && c.projects.length > 0
      ? `In your project "${c.projects[0].title}", how did you architect data flow between client and server, and what was your toughest technical hurdle?`
      : `Describe a complex bug you encountered in a recent project and explain your step-by-step debugging strategy.`,
    missingRequired.length > 0
      ? `This role requires practical work with ${missingRequired[0]}. What is your current familiarity, and how would you ramp up within your first sprint?`
      : `How do you ensure code maintainability, error handling, and security when building backend API endpoints?`,
...
function runHeuristicBiasCheck(text: string) {
  const flags: any[] = [];
  const lower = text.toLowerCase();

  // Check education elitism
  if (
    ...
    lower.includes("nit") ||
    lower.includes("bits") ||
    lower.includes("top university only") ||
    lower.includes("premier institution")
  ) {
    flags.push({
      category: "education",
      severity: "high",
  ...
  // Check internship experience inflation
  if (
    lower.includes("3+ years") ||
    lower.includes("3-5 years") ||
    lower.includes("2+ years") ||
    lower.includes("4+ years") ||
    lower.includes("prior industry experience required")
  ) {
    flags.push({
      category: "experience",
      severity: "high",
  ...
  // Check aggressive/gendered language
  ...
  // Check hyper-specific proprietary tools
  ...
  return {
    ...
    summary: flags.length > 0 
      ? `Identified ${flags.length} potential exclusionary barriers in the Job Description, primarily regarding institutional tiering and experience expectations for an intern position.`
      : "The Job Description has an inclusive, accessible tone, focusing on practical competency and engineering potential.",
    flags,
    improvedJD: improved
  };
}

function generateLocalRecruiterResponse(query: string, candidates: any[] = []): string {
  const q = query.toLowerCase();

  // Comparison query: "Why is X ranked above Y?"
...
${top.map((c, i) => `${i + 1}. **${c.name}** (Overall Rank #${c.rank}, Score: ${c.finalScore}%)
   ...
   - Key Backend Skills: ${c.matchedExplicitSkills?.filter((s: string) => ["Node.js", "Express", "REST APIs", "PostgreSQL", "MongoDB", "SQL"].includes(s)).join(", ") || "REST APIs, Node.js"}`).join("\n\n")}
...
function runHeuristicJDParser(fileName: string, rawText: string) {
  ...
  if (!title) {
    title = fileName
      .replace(/\.[^.]+$/, "")
      .replace(/[_\-]+/g, " ")
      .replace(/\b(?:jd|job|description)\b/gi, "")
      .trim();
  ...
  // Section slicing to separate required from preferred
  ...
  TECH_SKILLS.forEach(skill => {
    ...
    if (isPresent) {
      ...
      const isExclusivelyPreferred = preferredSectionIndex !== -1 &&
        skill.aliases.some(alias => preferredText.includes(alias)) &&
        !skill.aliases.some(alias => requiredText.includes(alias));
  ...
  // Guarantee minimum core requirements
  ...
  // 5. Extract Responsibilities & Qualifications lines
...
function runHeuristicResumeParser(fileName: string, rawText: string) {
  const lines = rawText.split("\n").map(l => l.trim()).filter(Boolean);
  const lower = rawText.toLowerCase();
  ...
  // 4. Skills extraction from comprehensive taxonomy
  const ALL_SKILLS: { name: string; aliases: string[] }[] = [
  ...
  const experience: { title: string; company: string; period: string; description: string }[] = [];
